...
class MetaLenetDECOLLE(LenetDECOLLE, MetaModuleNg):
    def __init__(
        self, burnin, detach_at=-1, sg_function_baseline=False, *args, **kwargs
    ):
    ...
    def build_conv_stack(
        ...
        for i in range(self.num_conv_layers):
            ...
            if self.lc_ampl is not None:
                readout = nn.Linear(
                    int(feature_height * feature_width * Nhid[i + 1]), out_channels
                )
    ...
    def step(self, input, params=None):
        ...
        u_out = []
        i = 0
        for lif, pool, ro, do in zip(
            self.LIF_layers, self.pool_layers, self.readout_layers, self.dropout_layers
        ):
            if i == self.num_conv_layers:
                input = input.view(input.size(0), -1)
            s, u = lif(
                input, self.get_subdict(params, "LIF_layers.{0}.base_layer".format(i))
            )
            if i == self.detach_at:
...
class MetaRecLIFLayer(LIFLayer, MetaModuleNg):
    def __init__(*args, **kwargs):
        raise NotImplementedError(
            "See previous commits of pytorch-maml:update__kennetms"
        )
...
#     neuron_params_drop = {
#         **neuron_params,
#         "dropout": slayer.neuron.Dropout(p=0.0),
#     }
#     neuron_params_final = {
#         "threshold": 1.0,
#         "current_decay": 0.10,
...
#                     neuron_params=self.neuron_params_drop,
#                     in_features=2,
#                     out_features=16,
#                     kernel_size=5,
#                     delay=False,
...
#                     neuron_params=self.neuron_params, kernel_size=2, stride=2
#                 ),
#                 slayer.block.cuba.Conv(
#                     neuron_params=self.neuron_params_drop,
#                     in_features=16,
...
#                 ),
#                 slayer.block.cuba.Dense(
#                     neuron_params=self.neuron_params_final,
#                     in_neurons=512,
...
#             spike = block(
#                 spike, params=self.get_subdict(params, "blocks.{0}".format(block_id))
#             )
...
#                     weights = (
#                         slayer.utils.quantize(b.synapse.weight, step=2)
#                         .cpu()
#                         .data.numpy()
#                     )
...
#                 if b.delay:
#                     delays = (
#                         slayer.utils.quantize(b.delay.delay, step=2).cpu().data.numpy()
#                     )
...
def build_model_DECOLLE(
    out_features, params_file, device, detach_at, sg_function_baseline
):
    ...
    net = MetaLenetDECOLLE(
        ...
        detach_at=detach_at,
        sg_function_baseline=sg_function_baseline,
    ).to(device)
    ...
    ##Initialize
    net.init_parameters(
        torch.zeros([1, params["chunk_size_train"]] + params["input_shape"]).to(device)
    )

    return net


# def build_model_SLAYER(
#     out_features, params_file, device, network_type="conv"
# ):  # = 'maml/decolle_params-CNN.yml'
#     from decolle.utils import parse_args, prepare_experiment, cross_entropy_one_hot
#     import datetime, os, socket, tqdm
#     import torch
...
#         net = SLAYERDenseNetwork(out_features, input_shape[1], input_shape[2]).to(
#             device
#         )
...
def build_model_REDECOLLE(
    out_features, params_file, rec_layer=MetaRecLIFLayer, out_layer=MetaLIFLayer
):
    from decolle.utils import parse_args, prepare_experiment, cross_entropy_one_hot
    import datetime, os, socket, tqdm
    import torch
    ...
    net = MetaLenetREDECOLLE(
        ...
        num_mlp_layers=params["num_mlp_layers"],
        lc_ampl=params["lc_ampl"],
        lif_layer_type=[MetaLIFLayer] * len(params["Nhid"])
        + [rec_layer] * len(params["Mhid"])
        + [out_layer],
        method=params["learning_method"],
        with_output_layer=True,
        wrp=params["wrp"],
    ...
    net.init_parameters(
        torch.zeros([1, params["chunk_size_train"]] + params["input_shape"]).cuda()
    )
