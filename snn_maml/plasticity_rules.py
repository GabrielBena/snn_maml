...
def loihi_soel(
    ...
    for name, param in params.items():
        ...
        if name != names[-1]:
            ...
            # pdb.set_trace()
            updated_params[name] = (
                param + step_size * model.blocks[-1].synapse.weight.grad
            )
...
def maml_soel(
    model, U, targets, params=None, step_size=0.5, first_order=False, threshold=None
):
    ...
    if threshold is None:
        threshold = torch.Tensor([0.05], requires_gradient=False).to(
            model.get_input_layer_device()
        )
    ...
    if hasattr(model, "LIF_layers"):
        param_name_weight = "LIF_layers.{0}.base_layer.weight".format(
            len(model.LIF_layers) - 1
        )
        param_name_bias = (
            None  #'LIF_layers.{0}.base_layer.bias'.format(len(model.LIF_layers)-1)
        )
...
def custom_sgd(
    model, loss, params=None, step_size=0.5, first_order=False, custom_update_fn=None
):
        ...
        for (name, param), grad in zip(params.items(), grads):
            if grad is not None:
                if custom_update_fn is not None and "weight" in name:
                    deltaw = custom_update_fn(
                        grad, params[name].data, eta=step_size[name]
                    )
...
def custom_sgd_reg(
    model, loss, params=None, step_size=0.5, anchor_params=None, lamda=1.0
):
    """Update of the meta-parameters with one step of gradient descent on the
    loss function.
    """
    ...
    grads = torch.autograd.grad(
        loss, params.values(), create_graph=True, allow_unused=True
    )
