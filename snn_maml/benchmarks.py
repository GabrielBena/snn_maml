...
from torchvision import (
    transforms as trn,
)  # .transforms import ToTensor, Resize, Compose
...
def get_benchmark_by_name(
    ...
    dataset_transform = ClassSplitter(
        shuffle=True, num_train_per_class=num_shots, num_test_per_class=num_shots_test
    )
    ...
    if name == "omniglot":
        ...
        if params_file is None:
            params_file = folder + "/parameters/decolle_params-CNN.yml"
        ...
        chunk_size = params["chunk_size_train"] if chunk_size is None else chunk_size
        dt = params["deltat"] if dt is None else dt
        transform = Compose(
            [
        ...
        meta_train_dataset = ClassSplitter(
            DoubleNMNIST(
                root=data_dir,
                meta_train=True,
                transform=transform,
        ...
        meta_val_dataset = ClassSplitter(
            DoubleNMNIST(
                root=data_dir,
                meta_val=True,
                transform=transform,
        ...
        meta_test_dataset = ClassSplitter(
            DoubleNMNIST(
                root=data_dir,
                meta_test=True,
                transform=transform,
        ...
        if "cuba" in params.keys():
            if params["cuba"]:
                model = build_model_DECOLLECUBA(
                    num_ways,
                    params_file=params_file,
                    device=device,
                    detach_at=detach_at,
                    sg_function_baseline=non_spiking,
                )
            else:
                model = build_model_DECOLLE(
                    num_ways,
                    params_file=params_file,
                    device=device,
                    detach_at=detach_at,
        ...
        from torchneuromorphic.double_dvssign.doubledvssign_dataloaders import (
            DoubleDVSSign,
            Compose,
            ClassDVSSignDataset,
            CropDims,
        ...
        pdb.set_trace()
        data_dir = (
            folder + "/../data/nomniglot/nomniglot.hdf5"
        )  # folder+'/data/nomniglot/nomniglot.hdf5'
        ...
        if "lava" in name:
            ...
            if params["network"]["analog_readout"]:
                loss_function = (
                    F.cross_entropy
                )  # slayer.loss.SpikeMax(mode='logsoftmax').to(device) #F.cross_entropy #slayer.loss.SpikeMax(mode='logsoftmax') # SpikeMax functions like NLL for classification
            else:
                loss_function = slayer.loss.SpikeMax(mode="logsoftmax").to(device)

            model = build_model_lava(
                num_ways, params_file=params_file, device=device, detach_at=detach_at
            )
        else:
            loss_function = F.cross_entropy
        ...
        from torchneuromorphic.dvs_gestures_torchmeta.dvsgestures_dataloaders_meta import (
            DVSGestureMeta,
            Compose,
        ...
        data_dir = (
            folder + "/../data/dvs_gesture_meta.hdf5"
        )  # folder+'/data/nomniglot/nomniglot.hdf5'
        ...
        meta_train_dataset = DVSGestureMeta(
            root=data_dir,
            meta_train=True,
            transform=transform,
            target_transform=target_transform,
        ...
        meta_val_dataset = DVSGestureMeta(
            root=data_dir,
            meta_test=True,
            transform=transform,
            target_transform=target_transform,
        ...
        meta_test_dataset = DVSGestureMeta(
            root=data_dir,
            meta_test=True,
            transform=transform,
            target_transform=target_transform,
        ...
        from torchneuromorphic.emg_meta.emg_dataloaders_meta import (
            EMGMeta,
            Compose,
            ClassEMGMetaDataset,
            CropDims,
        ...
        if "lava" in name:
            ...
            model = build_model_DECOLLE(
                ...
                detach_at=detach_at,
                sg_function_baseline=non_spiking,
            )  # ,detach=detach)#hidden_size=hidden_size)
