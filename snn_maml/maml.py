...
class ModelAgnosticMetaLearning(object):
    ...
    def __init__(
        ...
        if per_param_step_size or boil:
            ...
            if boil:
                assert (
                    learn_step_size is False
                ), "boil is not compatible with learning step sizes"
    ...
    def get_outer_loss(self, batch, stream_mode=False):
        ...
        results = {
            "num_tasks": num_tasks,
            "inner_losses": np.zeros(
                (self.num_adaptation_steps, num_tasks), dtype=np.float32
            ),
            ...
            with torch.set_grad_enabled(self.model.training):
                if stream_mode:
                    test_logits = self.model(
                        test_inputs.squeeze().transpose(0, 1), params=params
                    )
    ...
    def train(self, dataloader, max_batches=500, verbose=True, epoch=-1, **kwargs):
        with tqdm(total=max_batches, disable=False, **kwargs) as pbar:
            for results in self.train_iter(
                dataloader, max_batches=max_batches, epoch=epoch
            ):
                pbar.update(1)
                postfix = {"loss": "{0:.4f}".format(results["mean_outer_loss"])}
                if "accuracies_after" in results:
                    postfix["after"] = "{0:.4f}".format(
                        np.mean(results["accuracies_after"])
                    )
                if "accuracies_before" in results:
                    postfix["before"] = "{0:.4f}".format(
                        np.mean(results["accuracies_before"])
                    )
    ...
    def evaluate(self, dataloader, max_batches=500, verbose=True, **kwargs):
        mean_outer_loss, mean_accuracy, count = 0.0, 0.0, 0
        with tqdm(total=max_batches, disable=False, **kwargs) as pbar:
            for results in self.evaluate_iter(dataloader, max_batches=max_batches):
                ...
                if "accuracies_after" in results:
                    mean_accuracy += (
                        np.mean(results["accuracies_after"]) - mean_accuracy
                    ) / count
                    postfix["after in-loop"] = "{0:.4f}".format(np.mean(mean_accuracy))
                if "accuracies_before" in results:
                    postfix["before in-loop"] = "{0:.4f}".format(
                        np.mean(results["accuracies_before"])
                    )
                pbar.set_postfix(**postfix)
