...
def hessian_vector_product(inner_loss, vector, params):
    ...
    hvp2 = torch.autograd.grad(
        flat_grad, params, create_graph=False, retain_graph=True, grad_outputs=vec
    )
...
def create_fixed_quantizers():
    ...
    fixed_quantizers = {}
    fixed_quantizers["16"] = quantizer(
        forward_number=FixedPoint(wl=16, fl=15, clamp=True),
        forward_rounding="nearest",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )
    fixed_quantizers["8"] = quantizer(
        forward_number=FixedPoint(wl=8, fl=7, clamp=True),
        forward_rounding="nearest",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )

    fixed_quantizers["4"] = quantizer(
        forward_number=FixedPoint(wl=4, fl=3, clamp=True, symmetric=True),
        forward_rounding="nearest",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )
    fixed_quantizers["3"] = quantizer(
        forward_number=FixedPoint(wl=3, fl=1, clamp=True, symmetric=True),
        forward_rounding="nearest",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )

    fixed_quantizers["2"] = quantizer(
        forward_number=FixedPoint(wl=2, fl=1, clamp=True, symmetric=True),
        forward_rounding="nearest",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )

    fixed_quantizers["16s"] = quantizer(
        forward_number=FixedPoint(wl=16, fl=15, clamp=True),
        forward_rounding="stochastic",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
    )
    fixed_quantizers["8s"] = quantizer(
        forward_number=FixedPoint(wl=8, fl=7, clamp=True),
        forward_rounding="stochastic",
        backward_rounding="stochastic",
        clamping_grad_zero=True,
